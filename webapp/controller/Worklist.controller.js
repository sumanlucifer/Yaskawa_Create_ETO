	...
	return BaseController.extend("com.yaskawa.ETOWorkFlow.controller.Worklist", {
		...
		onBeforeUploadStarts: function (oEvent) {
			...
			var oCustomerHeaderSlug = new sap.m.UploadCollectionParameter({
				...
				value: this.fileContent + "|" + this.vbeln + "|" + oEvent.getSource()._aFilesFromDragAndDropForPendingUpload[lcv].name + "|" +
					oEvent.getSource()._aFilesFromDragAndDropForPendingUpload[lcv].type + "|" + oEvent.getSource()._aFilesFromDragAndDropForPendingUpload[
						lcv].size
				...
				for (var i = 0; i < this._allItems.length; i++) {
					if (this._allItems[i].getFileName() === sUploadedFileName) {
						...
						if (oEvent.getParameter("files")[j].status === 201) {
							...
							_that.getAttachments();
							_that._getAddAttachments().close();
						} else {
